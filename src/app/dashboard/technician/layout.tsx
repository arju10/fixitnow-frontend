...
import { LayoutDashboard, Calendar, Wrench, Clock, User, LogOut, Settings } from 'lucide-react';
import { cn } from '@/lib/utils';

const navItems = [
  { href: '/dashboard/technician', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/dashboard/technician/bookings', label: 'My Bookings', icon: Calendar },
...
export default function TechnicianDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && isAuthenticated && role !== 'TECHNICIAN') {
      router.push('/');
    }
            ...
            <div className="sticky top-20 rounded-xl border bg-card p-4 shadow-sm">
              <div className="space-y-1">
                {navItems.map((item) => {
                  ...
                  const isActive =
                    pathname === item.href ||
                    (item.href !== '/dashboard/technician' && pathname?.startsWith(item.href));
